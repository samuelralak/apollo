...
const ZapPortal = ({pubkey, eventId, eventCoordinate}: Props) => {
    ...
    const onZapSubmit: SubmitHandler<ZapSchemaType> = async ({amount, comment}) => {
        const recipient = ndkInstance().getUser({npub: nip19.npubEncode(pubkey)})
        const millisats = amount * 1000 // convert to millisats

        const zapper = new NDKZapper(recipient, millisats, 'msat', {
            comment,
            tags: eventTags(eventId, eventCoordinate),
        })
        zapper.ndk = ndkInstance()

        // Get LN invoice from zapper
        let invoiceFromRequest: string | null = null
        zapper.on('ln_invoice', ({pr}) => {
            invoiceFromRequest = pr
        })
